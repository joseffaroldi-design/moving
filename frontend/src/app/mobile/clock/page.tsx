...
export default function MobileClockPage() {
  ...
  const since = state.clock_in_at
    ? new Date(state.clock_in_at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })
    : null;
    ...
    <div>
      ...
      <div className="rounded-md border border-slate-200 bg-white p-6 text-center shadow-card">
        ...
        <p className="mt-3 font-heading text-lg font-bold text-navy">
          {loading ? "Loading…" : state.clocked_in ? "On the clock" : "Not clocked in"}
        </p>
        {since && <p className="text-sm text-slate-500">Since {since}</p>}
        {state.clocked_in && state.job_id && (
          <p className="mt-1 text-xs text-slate-400">Linked to this move</p>
        )}
      </div>

      <div className="mt-4">
        {state.clocked_in ? (
          <Button variant="danger" size="lg" className="w-full py-4 text-base" onClick={clockOut} disabled={loading || saving} data-testid="clock-out-button">
            <LogOut className="h-5 w-5" /> {saving ? "Clocking out…" : "Clock Out"}
          </Button>
        ) : (
          <Button variant="navy" size="lg" className="w-full py-4 text-base" onClick={clockIn} disabled={loading || saving} data-testid="clock-in-button">
            <LogIn className="h-5 w-5" /> {saving ? "Clocking in…" : "Clock In"}
          </Button>
        )}
      </div>

      {jobId && (
        <Link href={`/mobile/jobs/${jobId}`} className="mt-4 block text-center text-sm font-semibold text-gold-hover">
          Back to job
        </Link>
      )}
    </div>
